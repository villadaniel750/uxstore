                ...
                <v-slide-group-item v-for="(img, index) in images" :key="index" v-slot="{ isSelected, toggle, selectedClass }">
                    <v-card :class="['ma-4', selectedClass]" width="260" flat tile color="transparent">
                        <div v-if="loading[index]">
                            <v-skeleton-loader height="200" :type="`card-avatar, list-item-avatar-two-line`" class="mx-auto">
                        ...
                        <div v-else>
                            <CxVideo :lazy-src="img.lazySrc" :src="img.src" :blocked="index % 3 === 1" :is-playlist="index % 4 === 1" :playlist-count="index + 1" :title="'One meets his destiny on the road he takes to avoid it destiny on the road he takes to avoid it destiny on the road he takes to avoid it destiny on the road he takes to avoid it'" :username="'Shamus'" :upload-date="'2024/10/7'" :profile-badge="index % 3" />
                        </div>
                    </v-card>
                </v-slide-group-item>
            </v-slide-group>
        ...
        <v-sheet class="mx-auto" width="100%" color="transparent" elevation="0">
            ...
            <v-slide-group v-model="model" selected-class="bg-success" :show-arrows="!mobileView">
                <v-slide-group-item v-for="(img, index) in images" :key="index" v-slot="{ isSelected, toggle, selectedClass }">
                    <v-card :class="['ma-4', selectedClass]" width="260" flat tile color="transparent">
                        <div v-if="loading[index]">
                            <v-skeleton-loader height="240" :type="`card-avatar, list-item-avatar-two-line`" class="mx-auto">
                                <template v-slot:default>
                                    <v-card-text>
                                        <v-skeleton-loader type="image, list-item-avatar-two-line"></v-skeleton-loader>
                                    </v-card-text>
                        ...
                        <div v-else>
                            <CxVideo :lazy-src="img.lazySrc" :src="img.src" :blocked="index%3===1" :isPlaylist="index%4===1" :playlist-count="index + 1" title="One meets his destiny on the road he takes to avoid it destiny on the road he takes to avoid it destiny on the road he takes to avoid it destiny on the road he takes to avoid it" username="Shamus" uploadDate="2024/10/7" :profileBadge="index%3" />
                        </div>
                    </v-card>
                </v-slide-group-item>
            </v-slide-group>
